...
async function createMeetingFn({
  ...
  try {
    ...
    return (
      `Done! Scheduled "${title}"${meetingType ? ` (${meetingType})` : ""} for ${department.name} on ${formattedTime} (${duration} mins). Hosted by ${actualHostName}.${locationInfo} I've sent a Google Calendar invite with the Zoom link to everyone.`
    );
  } catch (error) {
...
// ---------------------------------------------------------------------------
// checkAvailability — Check if specific people are free at a given time
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// findMutualTime — Find overlapping free time for a group of people
// ---------------------------------------------------------------------------
async function findMutualTimeFn({
  ...
  const validSlots = mergedSlots.filter(block => {
    ...
    return blockDurationMs >= Number(duration) * 60 * 1000;
  });
